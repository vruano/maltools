package MalariaGEN::AGV::Pipeline;
...
use MalariaGEN::AGV::Manifest::Pipeline;
...
has manifest => (is => 'ro', isa => 'MalariaGEN::AGV::Manifest::Pipeline', required => 1);
...
sub _BUILDARGS_process_manifest {
  ...
  elsif (blessed($manifest) && $manifest->isa('MalariaGEN::AGV::Manifest')) {
...
sub class_by_name {
  ...
  if ($name =~ /^MalariaGEN::AGV::Pipeline/) {
    eval "require $name" and return $name;
    return undef;
  }
  elsif ($name =~ /^::/) {
    $name = 'MalariaGEN::AGV::Pipelines' . $name;
    eval "require $name" and return $name;
    return undef;
  }
  elsif ($name =~ /::/) {
    ...
    $name = 'MalariaGEN::AGV::Pipelines::' . uc(substr($name,0,1)) . substr($name,1);
    eval "require $name" and return $name;
    return undef;
  }
}
...
sub _manifest_class {
  ...
  unless ($manifest_attr->has_type_constraint) {
    return 'MalariaGEN::AGV::Manifest::Pileline';
  }
  else {
    my $candidate = $manifest_attr->type_constraint->name;
    return 'MalariaGEN::AGV::Manifest::Pipeline' unless $candidate;
...
sub _template_dir {
   my $result = MalariaGEN::AGV::Config->template_dir("pipeline",$_[0]->class);
   return $result; 
}
