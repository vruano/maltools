package MalariaGEN::AGV::Commands::CandidateSnps;
...
sub execute {
  ...
  if ($operation eq "create") {
    $tool = MalariaGEN::AGV::Tool->tool_by_name('vcfToVarLists');
    $outparam = "snpout";
  }
  elsif ($operation eq "merge") {
    $tool = MalariaGEN::AGV::Tool->tool_b_name('mergeVarLists');
  }
  elsif ($operation eq "props" || $operation eq "properties") {
    $tool = MalariaGEN::AGV::Tool->tool_by_name('varListsProperties');
  }
