package MalariaGEN::AGV::Alignment::BWA;
...
sub bwa {
  ...
  $ref = MalariaGEN::AGV::Reference->new(sequences => $ref) unless blessed($ref);
  $in = MalariaGEN::AGV::Alignment->new(file => $in, reference => $ref) unless blessed($in);
  $out = MalariaGEN::AGV::Alignment->new(file => $out, reference => $ref) unless blessed($out); 
...
sub regrouper_options {
   my $in = shift;
   my @rgs = @{$in->read_groups};
   my $rg = $#rgs >= 0 ? $rgs[0] : MalariaGEN::AGV::Alignment::ReadGroups->new();
