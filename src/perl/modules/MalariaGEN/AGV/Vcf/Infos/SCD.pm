package MalariaGEN::AGV::Vcf::Infos::SCD;

use strict;
use warnings;

use MalariaGEN::AGV::Vcf::CoverageAnalyzer;
use Moose;
...
has '+id' => (default => 'SCD');
has '+description' => (lazy => 1, default => sub { 'Indicates sample coverage at a particular depth thresholds (SCD), concretelly: ' . $_[0]->thr });
#has '+number' => ( default => '.');
has '+type' => ( default => 'Float');
has 'thr' => (is => 'ro', isa => 'Str|ArrayRef', default => sub { '2,5,10,15,20' });
...
sub calculate_values {
  ...
  foreach my $k (keys %$gtypes) {
    ...
    my $ad = $v->{AD} or next;
    my $total_ad = 0;
    foreach my $c (split(/,/,$ad)) { $total_ad += $c; }
    for (my $i = 0; $i <= $#thresholds; $i++) {
      $passed[$i]++ if $total_ad > $thresholds[$i];
    }
  }
  return [@passed];
}
...
sub _good_samples_hash_builder {
  ...
  my %included = ();
  if ($gs_file) {
     my $gs_fh = IO::File->new($gs_file,'r') or die "could not find the good sample file '$gs_file' from " . getcwd();
     while (my $line = <$gs_fh>) {
       next if $line =~ /^#/;
