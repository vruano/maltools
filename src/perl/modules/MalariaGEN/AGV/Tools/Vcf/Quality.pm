package MalariaGEN::AGV::Tools::Vcf::Quality;
...
my $analizer = MalariaGEN::AGV::Vcf::QualityAnalyzer->new(gff => $gff_file, filter_all => $skip_filtered); 
...
foreach my $s (keys %$counters) \{
 ...
 foreach my $k (keys %$sample_counters) \{
   my $out_file_prefix = catfile($outdir,"qual-$k");
