package Maltools::Workflow;
use strict;
use warnings;
use Moose;
use Scalar::Util;
use Maltools::Workflow::Step;
...
sub _process_locals {
  ...
  my @result = map {
    ...
    return Maltools::Workflow::Var->new(name => $_, type => "local") unless blessed($_);
    $_->isa("Maltools::Workflow::Var") && $_->type eq "local" or die "wrong local class or type $_";
    return $_; 
  } @{$locals || []};
...
sub _vars_builder {
  ...
  my @in_vars = map {
    return Maltools::Workflow::Var->new(name => $_->name, type => 'in');
  } @{$inputs || []}; 
  my @out_vars = map {
    return Maltools::Workflow::Var->new(name => $_->name, type => 'out');
  } @{$outputs || []};
...
sub _step_vars {
  ...
  foreach my $in ($step->tool->inputs) {
    my $name = $in->name;
    push @result, Maltools::Workflow::Var->(name => $name, step => $step, type => 'in');
  }
  foreach my $out ($step->tool->outputs) {
    my $name = $out->name;
    push @result, Maltools::Workflow::Var->(names => $name, step => $step, type => 'out');
  }
  return wantarray ? @result : \@result;
}
...
sub _process_steps {
  ...
  my $next_step_num = 0;
  my @result = map { 
    $_ = Maltools::Workflow::Step->new(%$_) unless blessed($_);
    $_->isa('Maltools::Workflow::Step') or die "step has wrong class " . ref($_);
