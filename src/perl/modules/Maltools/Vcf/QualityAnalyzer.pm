package Maltools::Vcf::QualityAnalyzer;
...
has 'filter_all' => (is => 'ro', isa => 'Bool', default => 1);
has 'per_sample' => (is => 'ro', isa => 'Bool', default => 1); # create counters per sample.
...
sub increase_counters {
  ...
  my $total_delta_ln = int(log($total_delta-2) * 100); # total_delta must be 3 at least.
  foreach my $cn (@$counters) {
    ...
    foreach my $t ( @$types ) {
      ...
      if ($total_delta > $ct->{max}) {
        for (my $i = $ct->{max} + 1; $i <= $total_delta_ln; $i++) {
           $ct->{frequencies}->[$i] = 0; 
        }
        $ct->{max} = $total_delta;
      }
      $ct->{frequencies}->[$total_delta_ln]++;
...
sub calculate_percentiles {
  ...
  for (my $i = 0; $i < scalar(@indexes); $i++) {
     ...
     my $floor_v = $self->skip_percentile(\$left_count,\$floor,\$last,\$next,\@frequencies);
     my $ceil_v = $self->skip_percentile(\$left_count,\$ceil,\$last,\$next,\@frequencies);
     my $value = $floor_v * ($index - $floor) + $ceil_v * ($ceil - $index);
     push @values, sprintf("%1.4f",exp($value / 100) + 2 );
  }
...
sub gff_iterator {
  ...
  my $rt = ref($gff);
  return Maltools::GffUtils::Iterator->new(file => $gff) unless $rt;
  if (blessed($gff)) {
    return $gff if $gff->isa('Maltools::GffUtils::Iterator');
  }
  return Maltools::GffUtils::Iterator->new(fh => $gff);
}
...
sub analyze {
  my ($self,@args) = @_;
  if ($self->per_sample) {
    return $self->analyze_per_sample(@args);
  }
  else {
    return $self->analyze_global(@args);
  }
}

sub analyze_per_sample {
  my ($self,$vcf,%options) = @_;
  my %all_samples = %{$self->zero_sample_counter_group()};
  my %per_sample = $options{sample_counters} ? (map { $_ => $self->zero_sample_counter_group() } $vcf->get_samples) : ();
  my $gff_iterator = $self->gff_iterator();
  while (my $line = $vcf->next_data_hash) {
   my ($chr,$pos,$filters,$qual,$gtypes) = @$line{qw(CHROM POS FILTER QUAL gtypes)};
   $filters = [ split(/;/,$filters) ] if ref($filters) eq "";
   next if $self->is_left_out($filters);
   my $chr_counter = $all_samples{$chr} || ($all_samples{$chr} = $self->zero_counter_group);
   my @counters = ('all',$chr);
   ...
   $self->increase_counters(1,$qual,\%all_samples,\@types,\@counters);
   foreach my $s (keys %$gtypes) {
      my $sample_gq = $$gtypes->{$s}->{GQ} || undef;
      next unless defined $sample_gq;
      $self->increase_counters(1,$sample_gq,$per_sample{$s},\@types,\@counters) if exists $per_sample{$s};
   }
  }
  foreach my $s (keys %per_sample) {
    foreach my $k (keys %{$per_sample{$s}}) {
      $self->calculate_derivable($per_sample{$s}->{$k});
    }
  }
...
sub analyze_global { 
  my ($self,$vcf) = @_;
  return $self->analyze_per_sample($vcf,sample_counters => 1);
}
