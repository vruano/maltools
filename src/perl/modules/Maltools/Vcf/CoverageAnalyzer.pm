package Maltools::Vcf::CoverageAnalyzer;
...
has 'per_sample' => (is => 'ro', isa => 'Bool', default => 0); # create counters per sample.
...
sub analyze_per_sample {
  ...
  while (my $line = $vcf->next_data_hash) {
   my ($chr,$pos,$filters,$info,$gtypes) = @$line{qw(CHROM POS FILTER INFO gtypes)};
   ...
   foreach my $s (keys %$gtypes) {
      ...
      my @freqs = split(/,/,$sample_ad);
      my $tfreq = 0;
      for (my $i = 0; $i <= $#freqs; $i++) {
        $tfreq += $freqs[$i];
      }
      $total_ad += $tfreq;
      $self->increase_counters(1,$tfreq,$per_sample{$s},\@types,\@counters) if exists($per_sample{$s});
   }
   $self->increase_counters(1,$total_ad,\%all_samples,\@types,\@counters);
  }
