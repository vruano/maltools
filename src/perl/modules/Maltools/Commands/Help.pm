package Maltools::Commands::Help;
...
sub command_summary_line {
  ...
  warn $@ if $@;
  $@ and return undef;
  $command->isa('Maltools::Command') or return undef;
...
sub command_list {
  my $self_file = $INC{'Maltools/Commands/Help.pm'};
  ...
  while (my $file = readdir($dh)) {
    ...
    push @result, 'Maltools::Commands::' . $1;
  }
  closedir($dh);
  return @result;
}
...
sub execute {
  ...
  while (my $cmd_name = shift(@args)) {
    ...
    my $cmd = Maltools::Command->instance($cmd_name);
    if (defined $cmd) {
