package Maltools::Commands::Align;
...
sub execute {
  ...
  $reference ||= Maltools::Config->new()->get_reference_sequences();
  ...
  my $job;
  unless ($force) {
    my $in_aln = Maltools::Alignment->new(file => $inputs[0]);
    my $in_ref = Maltools::Reference->new(sequence_file => $reference);
    if ($in_aln->valid_reference($in_ref)) {
       if ($in_aln->order ne 'coordinate') {
          $tool = Maltools::Tool->tool_by_name("picard-sortSam");
  ...
  unless ($tool) {
    $tool = Maltools::Tool->tool_by_name("align");
