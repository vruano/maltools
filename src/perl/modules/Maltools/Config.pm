...
Maltools::Config -- wired configuration file for the AGV pipeline
...
sub resolve_config {
  ...
  my $result = Maltools::Manifest::Config->new();
...
sub scratch_instance {
  ...
  for (my $i = 0; $i <= $#$areas; $i += 2) {
     ...
     my $class = "Maltools::Engines::Scratch::$type";
     eval "require $class" or
         die "could not resolve scratch area type '$type'";
     push @areas, $class->new(root => $root);
  }
  require Maltools::Engines::Scratch::Array
      or die "could not resolve scratch 'array' type";
  return Maltools::Engines::Scratch::Array->new(areas => \@areas, exists($data->{exclusions}) ? ( exclusions => $data->{exclusions}) : ());
}
...
sub engine {
  ...
  return Maltools::Engine->engine_by_name($engine);
}
