...
use Maltools::Alignment::ReadGroup;
use File::Spec::Functions qw(catfile);

has file => (is => 'ro', isa => 'Str', required => 1);
has reference => (is => 'ro', isa => 'Maybe[Maltools::Reference]');
...
has read_groups => (is => 'ro', isa => 'ArrayRef[Maltools::Alignment::ReadGroup]', lazy => 1, builder => '_build_read_groups');
...
sub _build_read_groups {
  ...
  return [ map { Maltools::Alignment::ReadGroup->new(line => $_, alignment => $self) } grep { index($_,'@RG') == 0 } @$hl ];
}
