...
@ReadFilters({ NotPrimaryAlignmentFilter.class, BadMateFilter.class, UnmappedReadFilter.class, UnmappedMateFilter.class})
@By(DataSource.READS)
...
public abstract class FragmentWalker<MapType, ReduceType> extends
	...
	protected Set<FragmentFilter> filters;
	...
	private void addToPendingStarts(GATKSAMRecord value) {
	...
	private GenomeLoc startLocationFrom(FragmentRecord value) {
		if (order == FragmentOrder.FRAGMENT_START)
			return startLocationFrom(value.getFirstRead());
		else {
			return startLocationFrom(value.getReferenceFirstRead());
		}
	}
	...
	private boolean addToSortedBuffer(FragmentRecord frecord) {
		...
		list.add(frecord);
		if (pendingLoc != null) {
			Integer startCount = pendingStarts.get(pendingLoc);
			if (startCount == null)
				throw new IllegalArgumentException("unexpected condition");
			if (startCount == 1)
				pendingStarts.remove(pendingLoc);
			else
				pendingStarts.put(pendingLoc, startCount - 1);
		}
		return true;
	}
