...
has 'samtrak' => (is => 'rw');
has 'config' => (is => 'ro', isa => 'Maltools::Manifest::Config', lazy => 1, builder => '_build_config' );


sub resolve_engine {
  my ($self) = @_;
  my $result = Maltools::Engine->engine_by_name($self->engine_name);
...
sub command_class_by_name {
  ...
  $command_name = 'Maltools::Commands::' . join("",map { uc(substr($_,0,1)) . substr($_,1) } @cnparts);
