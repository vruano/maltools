...
sub job {
  ...
  return Maltools::Job->new(tool => $self,%args);
}
...
sub add_input {
  ...
  if (scalar(@args) == 1) {
    ...
    my %args = @args;
    my $in = Maltools::ToolData->new(%args);  
...
sub _process_data {
  ...
  foreach my $k (keys %$data) {
     ...
     $data->{$k} = Maltools::ToolData->new(%$v);
  }
  return wantarray ? %$data : $data;
}
