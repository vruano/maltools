package Maltools::ToolData;

use strict;
use warnings;
use Scalar::Util qw(blessed);
use Maltools::ToolData::Type;
use Moose;
...
sub _process_type {
  ...
  if (ref($type) eq "") {
    return  Maltools::ToolData::Type->instance(name => $type);
  }
  elsif (ref($type) eq "HASH") {
    return Maltools::ToolData::Type->instance(%$type);
  }
  elsif (blessed($type)) {
    return $type if $type->isa('Maltools::ToolData::Type');
  }
